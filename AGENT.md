...
1. **Orient**: read `AGENT.md`, `docs/agent-rules.json`, `docs/agent-feedback.md`, `docs/user/design-harness-workflow.md`, and the relevant role prompt. Run `npm run agents:handoff:inbox -- --role=<role>` before planning and treat only its `requested` output as new work; do not infer work from a manual directory scan. Read `docs/project-context.json` when the task is standard/full.
