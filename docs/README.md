...
FC Moim은 FC Guppy 한 팀의 실제 운영을 위한 데스크톱 중심 축구·풋살 웹앱입니다. 치지직 테마의 현재 제품 골격을 기준으로 인증, 데이터 접근, 권한, 배포, 검증 체계를 안정화합니다.
...
- FC Guppy는 서버가 결정하는 유일한 팀이며 사용자가 팀을 생성·탐색·전환할 수 없습니다.
- 비로그인 사용자는 FC Guppy 공개 정보를 볼 수 있고, 헤더 로고를 눌러 로그인 화면으로 돌아갈 수 있습니다.
- 계정 식별과 클럽 멤버십은 분리합니다.
- FC Guppy 역할, 승인 상태, 프로필, OVR, 포인트는 멤버십 데이터에 속합니다.
- 서버 라우트는 클라이언트가 보낸 식별자 필드가 아니라 현재 Supabase 서버 사용자로 권한을 확인해야 합니다.
- 클라이언트의 `clubId`는 구형 API 호환 필드일 뿐 데이터 범위나 권한의 근거가 아닙니다.
...
- `src/config/server-team.ts`의 서버 전용 provider가 `TeamContext`를 결정합니다. 기본 single mode는 FC Guppy를 사용하고, multi mode는 공개 club 또는 approved membership을 검증합니다.
- Codex와 Agy의 경로·runtime·handoff 계약은 `docs/agent-boundaries.md`를 단일 진실로 사용합니다.
