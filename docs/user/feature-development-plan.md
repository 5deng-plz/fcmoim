...
- 계획서 작성 작업은 `infra` 소유의 `docs/**` 변경으로 처리하고, 기존 [feature-proposals.md](/Users/5deng/work/fcmoim/docs/feature-proposals.md)는 구현 시작 전 `docs/user/feature-proposals.md`로 이동합니다.
...
- 변경 소유자: `infra`.
- 검증: 문서 이동 후 `npm run harness:validate`를 실행하고, 필요 시 `npm run harness:guard:diff`로 docs 범위만 변경됐는지 확인합니다.
...
- 변경 소유자: `builder`.
- 검증: `npm run lint`, `npm run typecheck`, 관련 컴포넌트 테스트 추가/수정, SDD 기반 UI 테스트로 모바일 폭 홈/투표/모달 입력 확인.
...
- 변경 소유자: `builder`.
- 검증: `npm run lint`, `npm run typecheck`, 카드/레이더 렌더링 테스트, SDD 기반 UI 테스트로 모바일/데스크톱 레이아웃과 텍스트 넘침 확인.
...
- 변경 소유자: `builder` for UI/store/API/service/types/data changes.
- 검증: `npm run verify:baseline`, API 테스트, 필요한 경우 `npm run db:check`, SDD 기반 UI 테스트로 드래그/확정/읽기 화면 확인.
...
- 변경 소유자: `builder` for records UI/API/data changes.
- 검증: 집계 유닛 테스트, 빈 시즌/경기 없는 시즌/동률 랭킹 시나리오, SDD 기반 UI 테스트로 Records 탭 확인.
...
- 변경 소유자: `builder`.
- 검증: `npm run lint`, `npm run typecheck`, SDD 기반 UI 테스트로 iPhone급 폭과 데스크톱 폭에서 pull/swipe/dark mode 확인.
...
- Ownership:
  - `builder`: app, API, data, UI, tests
  - `infra`: docs, harness, CI, deployment configuration
  - `verifier`: tests and review
  - `orchestrator`: `docs/project-context.json`
- Quality gates:
  ...
  - Runtime UI changes require SDD-based deterministic UI evidence.
  - Verifier must report no blockers before completion.
- Product constraints:
