...
| `.agents/scripts/guard-evidence.mjs` | 런타임 evidence와 현재 변경 묶음의 일치 여부 검사 |
| `.agents/scripts/guard-review.mjs` | 독립 Review verdict 검사 |
| `docs/project-context.json` | 현재 작업, evidence, review 상태 |
...
3. 새 색상이나 상태 표현이 필요하면 기존 토큰을 먼저 찾습니다.
4. 필요한 토큰이 없으면 구현보다 먼저 토큰 추가 정책을 갱신합니다.
5. UI를 수정합니다.
6. Browser Use로 실제 화면을 열고 접근 가능한 이름, selector, 상태 표시, console error 여부를 확인합니다.
7. evidence에는 확인 대상, provider, artifact, 현재 변경 fingerprint를 남깁니다.
8. Review Agent가 diff, design rule, evidence를 확인합니다.
9. `harness:guard:handoff`, `harness:guard:prepush`, `harness:ci`가 통과해야 완료할 수 있습니다.
...
- full-stack 작업은 새 `feature` Agent를 만들지 않고 `docs/project-context.json`의 `activeWork.allowedPaths` lease로 처리합니다. lease는 task 단위로 `main-orchestrator`가 발급하며 forbidden path를 우회할 수 없습니다.
- guard 실패는 CI, git hook, guard script 안에서 자동 수정하지 않습니다. `main-orchestrator`가 실패 로그를 구현 Agent에게 돌려보내 최대 1-2회 재시도하고, 이후에도 실패하면 blocker로 남깁니다.
- `docs/project-context.json`의 `updateVersion`은 `main-orchestrator`가 상태를 갱신할 때마다 증가시킵니다. 이번 정책은 lightweight versioning이며 JSON Patch 또는 RFC 6902 도입을 의미하지 않습니다.
...
|---|---|
| `preCommit` | staged diff ownership, staged design guard, lint |
| `prePush` | full design guard, verify, evidence guard, review guard |
| `handoff` | diff ownership, full design guard, evidence guard, review guard |
| `ci` | harness validation, fixture regression, full design guard, evidence guard, review guard, verify |

`preCommit`의 일부 guard는 긴급 로컬 작업에서만 명시적 사유로 우회할 수 있습니다. `prePush`, `handoff`, `ci`에서는 우회할 수 없습니다.
...
- `status: "passed"`
- `provider`: 예를 들어 Browser Use in-app browser
- `target`: URL, selector, 접근 가능한 이름 등
- `artifacts`: assertion, screenshot, console check, API response 같은 검증 산출물 요약
- `worktreeFingerprint`: 현재 변경 묶음과 evidence가 같은 대상을 검증했다는 fingerprint
- `errors`: 발견된 runtime error 또는 빈 배열

Browser Use를 사용할 수 있는 Codex 환경에서는 Playwright를 기본 요구하지 않습니다. 이 프로젝트의 UI runtime evidence provider는 Browser Use입니다.

## Review Standard

Review는 구현자가 작성한 자기 선언이 아닙니다.

Review verdict는 다음을 포함해야 합니다.

- `reviewedBy`: active work agent와 다른 evaluator
- `reviewedAt`
- `evidenceReviewed`
- `checks`
- `worktreeFingerprint`
- blocker가 없는 `ready` verdict

Review Agent는 디자인 규칙, semantic slot, evidence 누락, runtime risk를 blocker로 다룹니다.
...
6. `npm run harness:ci`로 전체 경로를 확인합니다.
