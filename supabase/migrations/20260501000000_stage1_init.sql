...
do $$
begin
  create type public.membership_status as enum ('pending', 'approved', 'rejected', 'suspended', 'withdrawn');
exception when duplicate_object then null;
end $$;
...
create table if not exists public.team_memberships (
  ...
  stats jsonb not null default '{"attack":60,"defense":60,"stamina":60,"mentality":60,"speed":60,"manner":60}'::jsonb,
  ...
  residence text check (residence is null or char_length(btrim(residence)) between 1 and 80),
...
create table if not exists public.matches (
  ...
  red_leader_confirmed boolean not null default false,
  blue_leader_confirmed boolean not null default false,
  memo text,
...
create table if not exists public.match_teams (
  ...
  formation_slot integer check (formation_slot between 0 and 17),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (match_id, membership_id)
);
...
create table if not exists public.comments (
  id uuid primary key default gen_random_uuid(),
  target_type text not null check (target_type in ('match', 'schedule_poll_option')),
...
stable
as $$
  select private.membership_belongs_to_club(
    private.comment_target_club_id(target_type, target_id),
...
create index if not exists comments_target_created_idx on public.comments (target_type, target_id, created_at);
...
values (
  ...
  'FC Guppy',
  'fc-guppy',
  'FC Guppy는 함께 뛰고 성장하는 풋살 팀입니다. 정기 경기와 기록 관리를 통해 즐겁고 꾸준한 팀 문화를 만들어갑니다.'
)
