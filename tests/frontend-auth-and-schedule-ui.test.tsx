...
describe('v1.0 schedule and poll UX', () => {
  ...
  it('shows confirmed match details and tactics instead of the empty state on selected match dates', async () => {
    ...
    try {
      ...
      const scheduleOpsFocus = screen.getByTestId('schedule-ops-smartfocus');
      const scheduleCalendarFocus = screen.getByTestId('schedule-calendar-smartfocus');
      expect(scheduleOpsFocus).toHaveClass('opacity-100', 'pointer-events-auto', 'blur-0');
      expect(scheduleCalendarFocus).toHaveClass('opacity-100', 'pointer-events-auto', 'blur-0');
      fireEvent.pointerDown(screen.getByTestId('schedule-tactics-focus-zone'));
      expect(scheduleOpsFocus).toHaveClass('opacity-30', 'pointer-events-none', 'blur-[1px]');
      expect(scheduleCalendarFocus).toHaveClass('opacity-30', 'pointer-events-none', 'blur-[1px]');
      expect(scheduleOpsFocus).toHaveAttribute('inert');
      expect(scheduleCalendarFocus).toHaveAttribute('aria-hidden', 'true');
      fireEvent.pointerDown(screen.getByTestId('schedule-smartfocus-root'));
      expect(scheduleOpsFocus).toHaveClass('opacity-100', 'pointer-events-auto', 'blur-0');
      expect(scheduleCalendarFocus).not.toHaveAttribute('inert');
      fireEvent.pointerDown(screen.getByTestId('schedule-comments-focus-zone'));
      expect(scheduleCalendarFocus).toHaveClass('opacity-30', 'pointer-events-none', 'blur-[1px]');
...
describe('locker room team management UI', () => {
  ...
  it('shows the locker room market below the profile on my page', async () => {
    ...
    expect(screen.getByTestId('player-trait-card')).toHaveClass('h-[88px]', 'rounded-xl', 'border', 'shadow-inner', 'bg-surface-card', 'text-primary');
    expect(screen.getAllByTestId('locker-shop-trait-card')[0]).toHaveClass('h-[88px]', 'w-[104px]', 'shrink-0', 'snap-start', 'rounded-xl', 'border', 'shadow-inner', 'bg-surface-card', 'text-primary');
