...
describe('v1.0 auth provider UI', () => {
  ...
  it('shows public team browse feedback and returns to login', async () => {
    ...
    expect(screen.getByText('5월 정기전').closest('.rounded-lg')).toHaveClass('bg-event-match-bg', 'border-event-match-border');
  ...
  it('routes authenticated join attempts by selected team membership state', async () => {
    ...
    expect(switchClub).toHaveBeenCalledWith('club-1');
    expect(useAppStore.getState().showJoinForm).toBe(false);
    expect(useToastStore.getState().message).toBe('이미 이 팀의 멤버입니다.');
  });
...
describe('v1.0 schedule and poll UX', () => {
  ...
  it('shows weather and fine dust forecast on the upcoming match card', () => {
    ...
    const scheduleIcon = container.querySelector('img[src="/icons/svgrepo-football.svg"]');
    expect(scheduleIcon).toBeInTheDocument();
    expect(scheduleIcon).toHaveClass('animate-counter-spin');
    expect(screen.getByText('6월 6일 토 20:00')).toHaveClass('text-blue-team');
  ...
  it('renders finished match result, post-match comments, and settlement CTA for losing team members', async () => {
    ...
    try {
      ...
      useScheduleStore.setState({
        ...
        upcomingMatches: [
          {
            ...createCalendarMatch('finished-match', '종료된 경기', '2026-05-17T20:00:00.000+09:00', 'match'),
            status: 'finished',
            ourScore: 1,
            oppScore: 3,
      ...
      expect(resultField.closest('.bg-event-match-bg')).toBeInTheDocument();
  ...
  it('keeps confirmed match tactics visible when a poll exists on the same date', async () => {
    ...
    try {
      ...
      expect(screen.getAllByText('6월 정기전').length).toBeGreaterThan(0);
...
describe('records and header polish UI', () => {
  ...
  it('creates a manual schedule through the real match API flow', async () => {
    ...
    useScheduleStore.setState({
      ...
      upcomingMatchesError: null,
      calendarMatches: [
        createCalendarMatch('match-existing', '5월 정기전', '2026-05-20T20:00:00.000+09:00', 'match'),
