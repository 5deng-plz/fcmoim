...
export type CancelSchedulePollRequest = {
  clubId: string;
  pollId: string;
...
export async function cancelSchedulePoll(
  input: CancelSchedulePollRequest,
): Promise<SchedulePoll> {
  const response = await fetch('/api/schedule-polls/cancel', {
    method: 'POST',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
  ...
  if (!response.ok) {
    throw await buildApiError(response, '일정 투표를 취소하지 못했어요.');
  }
